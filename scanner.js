...
export class Scanner {
    ...
    static matchGtinBarcode(input) {
        if (!input)
            return null;
        // All 8 (EAN-8), 12 (UPC-A), 13 (EAN-13) characters must be digits
        if (![8, 12, 13].includes(input.length) || !/^\d*$/.test(input))
            return null;
        ...
        if (calculatedCheckDigit != suppliedCheckDigit)
            return null;
        return input;
    }
    ...
    static matchLibraryBarcode(input) {
        ...
        const suppliedCheckValue = suppliedCheckDigit == 'X' || suppliedCheckDigit == 'x'
            ? 10
            : parseInt(suppliedCheckDigit, 10);
        if (calculatedCheckValue != suppliedCheckValue)
            return null;
        return input.toUpperCase();
    }
    // Returns non-empty numeric input string, null otherwise
    static matchNumeric(input) {
        if (!input)
            return null;
        if (!/^\d+$/.test(input))
            return null;
    ...
    inputReceived(input) {
        ...
        if (duplicateAgeMs !== null &&
            duplicateAgeMs < this.options.duplicateRemovalMs) {
            return false;
        }
        for (const [category, matcherList] of Object.entries(this.options.categoryMatchers)) {
            for (const matcher of matcherList) {
                ...
                if (result !== null) {
                    ...
                    if (this.inputReceivedCallback(result, category) !== false) {
                        return true;
                    }
                }
            }
    ...
    keyDown(event) {
        ...
        if (this.options.verboseLog) {
            console.log(`KEY: @${new Date()
                .toISOString()
                .replace('T', ' ')
                .replace('Z', '')} #${event.keyCode} (${event.altKey}/${event.ctrlKey}/${event.metaKey}/${event.shiftKey}) =${event.key}`);
        }
        if (this.timerId !== null) {
        ...
        if (this.options.numpadAlwaysDigits &&
        ...
        else if (this.options.allowBackspace &&
            this.buffer != '' &&
            event.keyCode == 8) {
            // Backspace
            event.preventDefault();
    ...
    paste(event) {
        ...
        const newLines = [...text].reduce((sum, char) => sum + (char == '\n' ? 1 : 0), 0);
